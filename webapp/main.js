import init, {
  PolyominoSolver,
  get_predefined_polyomino,
} from './pkg/polyomino_tiling.js';
...
class PolyominoApp {
  ...
  setupEventListeners() {
    ...
    document
      .getElementById('board-width')
      .addEventListener('change', () => this.updateBoard());
    document
      .getElementById('board-height')
      .addEventListener('change', () => this.updateBoard());

    // Auto calculation checkboxes
    document
      .getElementById('auto-width')
      .addEventListener('change', (e) =>
        this.handleAutoWidth(e.target.checked)
      );
    document
      .getElementById('auto-height')
      .addEventListener('change', (e) =>
        this.handleAutoHeight(e.target.checked)
      );

    // Custom polyomino editor
    document
      .getElementById('add-mino-btn')
      .addEventListener('click', () => this.openEditor());
    document
      .getElementById('close-modal')
      .addEventListener('click', () => this.closeEditor());
    document
      .getElementById('editor-size')
      .addEventListener('change', (e) =>
        this.changeEditorSize(parseInt(e.target.value))
      );
    document
      .getElementById('clear-editor-btn')
      .addEventListener('click', () => this.clearEditor());
    document
      .getElementById('save-mino-btn')
      .addEventListener('click', () => this.saveCustomMino());

    // Close modal when clicking outside
    document
      .getElementById('custom-mino-modal')
    ...
    try {
      ...
      for (const pieceName of this.selectedPieces) {
        if (pieceName.startsWith('custom_')) {
          // Custom polyomino
          const customMino = this.customPolyominoes.find(
            (m) => m.name === pieceName
          );
          if (customMino) {
      ...
      if (solution === null) {
        this.updateStatus(
          'No solution found for the selected pieces and board size.',
          'error'
        );
        this.clearBoard();
      } else {
        this.updateStatus(
          `Solution found! Used ${solution.length} pieces.`,
          'success'
        );
  ...
  updateBoard() {
    ...
    if (this.selectedPieces.length > 0) {
      this.updateStatus(
        'Board dimensions changed. Click \'Solve Puzzle\' to solve again.'
      );
    }
  }
  ...
  updatePreview() {
    ...
    let minRow = this.editorSize,
      maxRow = -1;
    let minCol = this.editorSize,
      maxCol = -1;
  ...
  isConnected() {
    ...
    while (queue.length > 0) {
      ...
      for (const [dr, dc] of [
        ...
        if (
          newRow >= 0 &&
          newRow < this.editorSize &&
          newCol >= 0 &&
          newCol < this.editorSize &&
  ...
  saveCustomMino() {
    ...
    const hasActiveCells = this.editorGrid.some((row) =>
      row.some((cell) => cell)
    );
    if (!hasActiveCells) {
    ...
    if (
      this.customPolyominoes.some((m) => m.name === customName) ||
    ...
    this.updateStatus(
      `Custom polyomino "${name}" added successfully!`,
      'success'
    );
  }
  ...
  deleteCustomMino(fullName) {
    ...
    this.customPolyominoes = this.customPolyominoes.filter(
      (m) => m.name !== fullName
    );
